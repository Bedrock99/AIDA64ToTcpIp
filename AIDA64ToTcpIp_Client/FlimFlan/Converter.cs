	...
	public class Converter
	{
		...
		public static IconHolder BitmapToIconHolder(BitmapHolder bmp)
		{
			...
			if (mapColors)
			{
				...
				{
					//currently assumes source bitmap is 24bit color
			...
			else
			{
				...
				if (bmp.info.infoHeader.biBitCount == 32)
				{
					for (int i = 0; i < indexedImage.Length; i++)
					{
						//currently assumes source bitmap is 32bit color with alpha set to zero
				...
				{
					//implement converting an indexed bitmap
			...
			//is it really safe to assume the bitmap width/height are bytes?
			...
			// safe to assume that the first imageoffset is always 22
			...
			for (int i = 0; i < indexedImage.Length; i++)
			{
				...
				if (bitPosAND == 1)
				{
					...
					if (bytePosAND % 2 == 1)
					{
						//fix assumption that icon is 16px wide
		...
		private static RGBQUAD[] buildColorTable(Hashtable colors, ushort bpp)
		{
			...
			// it looks like the color array needs to be the max size based on bitcount
			int numColors = 1 << bpp;
